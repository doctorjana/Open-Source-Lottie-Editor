...
import { Folder, FileJson, Image as ImageIcon, Box, Shapes } from 'lucide-react';
import { IconPanel } from './IconPanel';

export function ProjectPanel() {
    const [activeTab, setActiveTab] = useState<'project' | 'icons'>('icons');
        ...
        <div className="h-full w-full bg-[#0a0a0a] border-r border-border/10 flex flex-col text-xs overflow-hidden">
            <div className="flex border-b border-border/10 shrink-0">
                <button
                    ...
                    className={`flex-1 h-10 flex items-center justify-center gap-2 transition-colors ${activeTab === 'project' ? 'bg-[#151515] text-blue-400 font-bold border-b-2 border-blue-500' : 'text-muted-foreground hover:bg-white/5'}`}
                >
                ...
                <button
                    ...
                    className={`flex-1 h-10 flex items-center justify-center gap-2 transition-colors ${activeTab === 'icons' ? 'bg-[#151515] text-blue-400 font-bold border-b-2 border-blue-500' : 'text-muted-foreground hover:bg-white/5'}`}
                >
